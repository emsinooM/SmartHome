...
// SSID and Password
const char *ssid = "UTE Wifi";
const char *password = "Bietpassroicho";

/**** NEED TO CHANGE THIS ACCORDING TO YOUR SETUP *****/
// The REST API endpoint - Change the IP Address
const char *base_rest_url = "http://192.168.147.31:5000/";
...
void sendLEDStatus(const char *objectId, const char *status)
{
  char rest_api_url[200];
  sprintf(rest_api_url, "%sapi/sensor/%s", base_rest_url, objectId);
  Serial.println(rest_api_url);
  ...
  serializeJson(doc, jsondata);
  Serial.println("JSON Data for LED...");
  Serial.println(jsondata);
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= readInterval)
  {
    ...
    //setLEDStatus(den_test_CBAS,convertStatus(light1Setup.status)); // Set light1 value
